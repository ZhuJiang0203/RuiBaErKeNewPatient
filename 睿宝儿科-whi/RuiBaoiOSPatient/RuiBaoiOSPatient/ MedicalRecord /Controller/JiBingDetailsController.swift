...
class JiBingDetailsController: BaseViewController {

    var model: JiBingLieBiao!
    
    var keys = ["\(kDisease)", "ICD", "\(kType)", "\(kDescribe)", "\(kStart)", "\(kEnd)", "\(kNoteTaker)"]
    var values = [String]()
    
    fileprivate var models = [JiBingDetailsModel]()
    ...
    fileprivate func setupData() {
        values = [model.jiBingName, model.ICD, model.type, model.descri, model.startTime, model.endTime, model.jiLuRen]
        
        for i in 0..<7 {
            let mdl = JiBingDetailsModel()
...
extension JiBingDetailsController: UITableViewDataSource, UITableViewDelegate {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let cell = JiBingDetailsCell(style: UITableViewCellStyle.default, reuseIdentifier: nil)
...
private class JiBingDetailsCell: UITableViewCell {
    
    var model: JiBingDetailsModel? {
        didSet{
            if model != nil {
...
private class JiBingDetailsModel: NSObject {
    var keyString = ""
    var valueString = ""
}
