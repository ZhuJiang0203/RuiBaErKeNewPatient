...
class GuoMinDetailsController: BaseViewController {

    var model: GuoMinLieBiao!
    
    fileprivate var keys = ["\(kTheAllergy)", "\(kType)", "\(kDegree)", "\(kReaction)", "\(kStart)", "\(kEnd)", "\(kNoteTaker)"]
    fileprivate var values = [String]()
    
    fileprivate var models = [GuoMinDetailsModel]()
    ...
    fileprivate func setupData() {
        values = [model.guoMinYuan, model.guoMinType, model.guoMinChengDu, model.fanYing, model.start, model.end, model.jiLuRen]
        
        for i in 0..<7 {
            let mdl = GuoMinDetailsModel()
...
extension GuoMinDetailsController: UITableViewDataSource, UITableViewDelegate {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let cell = GuoMinDetailsCell(style: UITableViewCellStyle.default, reuseIdentifier: nil)
...
private class GuoMinDetailsCell: UITableViewCell {
    
    var model: GuoMinDetailsModel? {
        didSet{
            if model != nil {
...
private class GuoMinDetailsModel: NSObject {
    var keyString = ""
    var valueString = ""
}
